...
from validator import login_mode_union_validate, instance_type_union_validate
...
def config(access_key_id, secret_access_key):
    ...
    ACCESS_KEY_ID 申请的API密钥ID

    SECRET_ACCESS_KEY 申请的API密钥
    """
...
def run_instances(image_id, login_mode, zone, instance_type, login_keypair, login_passwd, cpu, memory):
    ...
    try:
        ...
        return
    try:
        instance_type_union_validate(instance_type, cpu, memory)
    except click.BadParameter as e:
        click.echo(e)
        return
...
@cli.command()
@click.argument("zone", type=click.Choice(["pek3", "pek3a", "pek3b", "gd2", "sh1a", "ap2a"], case_sensitive=True))
@click.option("--instance-id", help="主机ID", multiple=True, required=True)
